...
public class HexFeatureManager : MonoBehaviour
{
    ...
    public void AddFeature(MainHexCell cell ,Vector3 position) 
    {
        ...
        otherPrefab = PickPrefab(
            plantCollections, cell.dataHexCell.featuresHexCell.PlantLevel, hash.c, hash.d
        );
        if (prefab)
        {
    ...
    public void AddWall(
        EdgeVertices near, MainHexCell nearCell,
        EdgeVertices far, MainHexCell farCell,
        bool hasRiver, bool hasRoad)
    {
        if (nearCell.dataHexCell.wallsScript.Walled != farCell.dataHexCell.wallsScript.Walled &&
            !nearCell.dataHexCell.waterScript.IsUnderwater && !farCell.dataHexCell.waterScript.IsUnderwater &&
            nearCell.brainHexCell.GetEdgeType(farCell) != HexEdgeType.Cliff)
        {
    ...
    public void AddWall(
    Vector3 c1, MainHexCell cell1,
    Vector3 c2, MainHexCell cell2,
    Vector3 c3, MainHexCell cell3
)
    {
        if (cell1.dataHexCell.wallsScript.Walled)
        {
            if (cell2.dataHexCell.wallsScript.Walled)
            {
                if (!cell3.dataHexCell.wallsScript.Walled)
                {
                    AddWallSegment(c3, cell3, c1, cell1, c2, cell2);
                }
            }
            else if (cell3.dataHexCell.wallsScript.Walled)
            {
        ...
        else if (cell2.dataHexCell.wallsScript.Walled)
        {
            if (cell3.dataHexCell.wallsScript.Walled)
            {
                AddWallSegment(c1, cell1, c2, cell2, c3, cell3);
    ...
    void AddWallSegment(
        Vector3 pivot, MainHexCell pivotCell,
        Vector3 left, MainHexCell leftCell,
        Vector3 right, MainHexCell rightCell
    )
    {
        if (pivotCell.dataHexCell.waterScript.IsUnderwater)
        {
            return;
        }

        bool hasLeftWall = !leftCell.dataHexCell.waterScript.IsUnderwater &&
            pivotCell.brainHexCell.GetEdgeType(leftCell) != HexEdgeType.Cliff;
        bool hasRighWall = !rightCell.dataHexCell.waterScript.IsUnderwater &&
            pivotCell.brainHexCell.GetEdgeType(rightCell) != HexEdgeType.Cliff;
        ...
        if (hasLeftWall)
        {
            ...
            else if (leftCell.dataHexCell.Elevation < rightCell.dataHexCell.Elevation)
            {
        ...
        {
            if (rightCell.dataHexCell.Elevation < leftCell.dataHexCell.Elevation)
            {
